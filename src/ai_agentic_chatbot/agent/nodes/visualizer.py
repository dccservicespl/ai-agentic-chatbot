...
class VisualizationNode:
    ...
    def determine_visualization(self, state: dict) -> dict:
        ...
        logger.info(f"[Visualizer] Data shape: {num_rows}x{num_cols}, columns: {columns}")
    ...
    def _apply_heuristics(self, df: pd.DataFrame, sql_query: str, explanation: str) -> dict:
        """Apply intelligent heuristics to determine visualization type."""
        num_rows, num_cols = df.shape
        columns = df.columns.tolist()
        sql_lower = sql_query.lower()
        
        # 1. Single Value (KPI) - Perfect for aggregated metrics
        ...
        # 3. Categorical Comparison (String + Numeric) -> Bar Chart
        if num_cols == 2 and num_rows <= 20:
            first_col = df.iloc[:, 0]
            second_col = df.iloc[:, 1]
            
            # Check if first column is categorical and second is numeric
            if (pd.api.types.is_string_dtype(first_col) or pd.api.types.is_object_dtype(first_col)) and \
               pd.api.types.is_numeric_dtype(second_col):
        ...
        # 4. Distribution/Percentage Data -> Pie Chart
        if num_cols == 2 and num_rows <= 8:
            second_col_name = columns[1].lower()
            if any(keyword in second_col_name for keyword in ['percent', 'percentage', 'share', 'proportion']):
                return self._create_payload(
                    type="pie_chart",
        ...
        # 5. Multiple Metrics (3+ columns, few rows) -> Table with highlights
        ...
        # 6. Large Dataset -> Table with pagination
        return self._create_payload(
            type="table",
            title="Query Results",
            data=df.head(100).to_dict('records'),  # Limit for performance
            summary=f"Showing first 100 of {num_rows} records." if num_rows > 100 else f"All {num_rows} records displayed.",
            config={
                "columns": columns,
                "total_rows": num_rows,
    ...
    def _format_kpi_value(self, value: Any, column_name: str) -> str:
        ...
        # Currency formatting
        if any(keyword in column_lower for keyword in ['sales', 'revenue', 'amount', 'price', 'cost', 'total', 'value']):
            return f"${value:,.2f}"
        ...
        # Count formatting
        if any(keyword in column_lower for keyword in ['count', 'number', 'qty', 'quantity']):
            return f"{int(value):,}"
    ...
    def _detect_value_format(self, value: Any, column_name: str) -> str:
        ...
        if any(keyword in column_lower for keyword in ['sales', 'revenue', 'amount', 'price', 'cost', 'total', 'value']):
            ...
            return "percentage"
        elif any(keyword in column_lower for keyword in ['count', 'number', 'qty', 'quantity']):
            return "integer"
        else:
            return "decimal"
    ...
    def _create_payload(self, type: str, title: str, data: List[Dict], 
                       summary: str = "", config: Optional[Dict] = None) -> dict:
