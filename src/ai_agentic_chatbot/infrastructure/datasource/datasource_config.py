...
from .datasource_types import DataSourceProvider, DataSourceType, DATASOURCE_CONFIG_REGISTRY
...
class PostgreSQLConfig(BaseDatasourceConfig):
    ...
    application_name: str = Field(default="ai_agentic_chatbot", description="Application name")
...
DATASOURCE_CONFIG_REGISTRY[DataSourceProvider.AWS_RDS_MYSQL] = MySQLConfig  # Reuse MySQL config
DATASOURCE_CONFIG_REGISTRY[DataSourceProvider.AWS_RDS_POSTGRESQL] = PostgreSQLConfig  # Reuse PostgreSQL config
...
def get_datasource_config_class(provider: DataSourceProvider):
    ...
    if provider not in DATASOURCE_CONFIG_REGISTRY:
        raise ValueError(f"No configuration class registered for datasource provider: {provider}")
